...
# 顺时针打印矩阵 todo 一定分情况考虑
def spiralOrder(matrix):
    def circle(matrix, x1, y1, x2, y2):
        res = []
        i, j = x1, y1
        if x1 < x2 and y1 < y2:
            while(j < y2):
    ...
    while(x1 <= x2 and y1 <= y2):
        res.extend(circle(matrix, x1, y1, x2, y2))
        x1 += 1
        y1 += 1
        x2 -= 1
        y2 -= 1
...
# 整数拆分   todo j是第一个拆出来的数，j不会再拆分！i-j可拆可不拆，取最大！
def integerBreak(n: int) -> int:
    dp = [1] * (n + 1)
    dp[2] = 1
    for i in range(n + 1):
        for j in range(1, i):
            cur = max(j * dp[i - j], j * (i - j))
            dp[i] = max(dp[i], cur)
    return dp[-1]

# K 个一组翻转链表 todo 最后不满足k的不反转！  一定注意几处边界
def reverseKGroup(head: ListNode, k: int) -> ListNode:
    # todo 返回新的head与tail 方便与后面相连
    def reverse(head):
        if head is None:
            return head
        pre = None
    ...
    # todo 开始不会为空，但是迭代结束的时候会None的
    if head is None:
        return None
    cur = head
    i = 0 # todo 当前就在0位置，走k-2次到k-1位置就是第k个，cur.next是新的head
    while(i < k - 1 and cur.next):
        i += 1
        cur = cur.next
    # todo 如果没走完就停了，说明不够了，直接返回
    if i < k - 1:
        return head
    newHead = cur.next
    cur.next = None
...
# 扑克牌中的顺子 todo 只有两个joker；最大值-最小值 < 5就可以，当然过滤掉重复的情况
...
def iou(preds, gt):
    ...
    left_top_y = np.maximum(preds[:, 1], gt[0])
...
def nms(preds, threshold):
    ...
    while(order.size > 0):
        ...
        left_top_x = np.maximum(x1[order[1:]], x1[maxIndex])
        left_top_y = np.maximum(x2[order[1:]], x2[maxIndex])
        right_bottom_x = np.minimum(y1[order[1:]], y1[maxIndex])
        right_bottom_y = np.minimum(y2[order[1:]], y2[maxIndex])
